...
// Columns
app.get("/api/columns", requireAuth, (_, res) =>
...
app.get("/api/tasks", requireAuth, (req, res) => {
  ...
  if (req.query.columnId)
    result = result.filter((t) => t.columnId === req.query.columnId);
  if (req.query.priority)
    result = result.filter((t) => t.priority === req.query.priority);
  if (req.query.assigneeId)
    result = result.filter((t) => t.assigneeId === req.query.assigneeId);
  if (req.query.search) {
...
app.get("/", (req, res) => {
  res.status(200).json({
    status: "healthy",
    message: "ProjectFlow API is live and running safely!",
    version: "1.0.0",
  });
});
